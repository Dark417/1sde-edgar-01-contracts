...
Ingest (repo 3) writes every batch to two sinks: S3 `fin-lake-raw` (system of
...
workspace may not be able to read `s3://fin-lake-raw` directly. The probe
...
dbutils.fs.ls("s3://fin-lake-raw/")  # works -> s3 mode is possible

# 2. Can the workspace read the managed volume? (expected: always yes)
dbutils.fs.ls("/Volumes/fin/landing/edgar/")
```
...
|---|---|---|
| S3 readable from the workspace | `s3` | Auto Loader reads `s3://fin-lake-raw/edgar/...`; the Volume sink can be disabled (one path deletes itself) |
| S3 not readable | `volume` | Auto Loader reads `/Volumes/fin/landing/edgar/...`; ingest keeps the dual sink; S3 remains the system of record and replay source |
...
- The value is published to SSM as `/fin-lakehouse/landing_mode` (repo 2) and
