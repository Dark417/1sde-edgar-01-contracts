...
3. **Full rename fin → edgar (2026-08-01)** — repos (`1sde-databricks-edgar-*`), package (`edgar_lakehouse_contracts`), catalog, buckets (`edgar-lake-raw/-serving`), SSM (`/edgar-lakehouse/*`). Done pre-publish so nothing downstream ever pins the old name. Old `fin` catalog verified empty, dropped.
