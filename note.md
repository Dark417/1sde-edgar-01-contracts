...
| Repo | State |
|---|---|
| 01-contracts | **DONE & RELEASED: v1.0.0** (v0.1.0 superseded), wheel on the GitHub release (public repo = the registry stand-in; S3 plan dropped). Release-on-tag pipeline: build -> liquibase update (CI, idempotent) -> publish. Only ADR-001 probe remains (needs raw bucket). |
...
## v1.0.0 close-out (2026-08-02)

Released and verified live, not just green in CI: 27 changesets applied, 13
tables match the consumer-verified shape, `liquibase status` reports up to date,
`edgar_lakehouse_contracts-1.0.0-py3-none-any.whl` attached to the GitHub
release. CI covers lint, mypy --strict, 100% coverage, the spark suite, drift
across all 13 tables (gold now in scope), secret scan, and liquibase validate.

**Only open item in repo 1: ADR-001.** Still `UNRESOLVED` because the probe
needs `s3://edgar-lake-raw` to exist, which is repo 2's first apply. Default
stays `volume`, which is correct either way.

**Blocking integration, but owned by other repos (global rule 11 — flagged, not
edited):**
- **repo 3** pins `==0.1.0` and uses three surfaces that all changed in 1.0.0:
  `LandingEnvelope` (all eleven field names), `landing_path` (partition key
  `dt=` -> `logical_date=`), and `FilingIndexRecord` (now requires
  `accession_number`, sliced from `file_name`). Until it moves, every landing
  file it writes is in a shape repo 4 cannot read.
- **repo 4** still vendors `src/pipelines/contracts/`. Deleting it and pinning
  1.0.0 is also what makes its contract-compat check able to fail.
- **repo 2** needs nothing: it mirrors only catalog/schema/bucket constants,
  which did not move. Separately: `backend.tf` still declares `dynamodb_table`,
  but Terraform 1.10 supports native S3 locking (`use_lockfile = true`) and the
  bootstrap has not run yet — cheapest possible moment to drop DynamoDB.

**Process note:** `git add -A` in the v1.0.0 commit silently deleted this file
(it had been moved out of the tree by another session). Stage explicitly when
two sessions share a working directory.
